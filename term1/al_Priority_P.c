...
static int	Do_CPU(int real)
{
	...
	if (running_cpu == -1)
	{
		...
		if (ready_queue.size == 0)
			return Do_CPU(1);
		// ready_queue에서 우선순위가 가장 큰 프로세스를 pop
		running_cpu = queue_pop_priority(1);
	...
	// 2.2. 기존 프로세스의 우선순위랑 ready_queue랑 다시 비교
	int min = queue_pop_priority(0);
	// 만약 ready_queue에 있는게 더 우선순위가 높으면 기존은 waiting이나 ready_queue에 보내고 Do_CPU(0)
